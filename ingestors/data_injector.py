...
def iterval_decorator(default_interval: int):
    def decorator(fetch_function: Callable[..., RetrievalResult]) -> Callable[..., RetrievalResult]:
    # Decorator that will respect
        ...
        def fetch_intervaled(*args: Any):
            ...
            time_to_sleep = (last_fetch_time + timedelta(seconds=interval) - datetime.utcnow()).total_seconds()
...
def get_max_page(link: str):
    last_link_re = re.compile(r'<https://api.github.com/events\?page=(\d+)[^>]*>; rel="last"', re.IGNORECASE)
...
def request_github_events(page: int, etag: str | None):
        ...
        if response.status_code != 200:
            raise ValueError(f'Got status code {response.status_code} with body {response.text}')

        etag = response.headers['ETag'] if 'ETag' in response.headers else None
        poll_interval = int(response.headers['X-Poll-Interval']) if 'X-Poll-Interval' in response.headers else None
        # If we didn't find a max page, we are on the last page
        max_page = get_max_page(response.headers['Link']) or page
        logging.debug(f'Got response {response.status_code} with etag {etag} and poll interval {poll_interval} and max page {max_page}')
        events: List[Event] = []
        for event in response.json():
            if event['type'] in ['WatchEvent', 'PullRequestEvent', 'IssuesEvent']:
                db_event = Event(id=int(event['id']),
                                 type=event['type'],
...
def get_new_events(last_event_id: int | None, etag: str | None, traverse_pages: bool = True):
    page = 1
    events: List[Event] = []
    # Dummy response
    response = RetrievalResult([], etag, 30, None)
    while response.max_page is None or page <= response.max_page and page == 1 or traverse_pages:
        ...
        new_events = (list(takewhile(lambda event: event.id != last_event_id, response.events))
            if last_event_id is not None else response.events)
    ...
    return RetrievalResult(events, response.etag, response.poll_interval, response.max_page), newest_event_id
