...
def iterval_decorator(default_interval: int):
    """
    Decorator that ensures the decorated function is not called more frequently than the specified interval.
    """
...
def request_github_events(page: int, etag: str | None):
    """
    Makes a GET request to the GitHub Events API and returns the relevant information.
    """
    headers = {}
    if etag is not None:
